...
class Robot:
    ...
    def __init__(self, motors: MotorGroup, urdf_path: str,
                 pybullet_client: Any, sim_conf: ml_collections.ConfigDict,
                 base_joint_names: Tuple[str,
                                         ...], foot_joint_names: Tuple[str,
                                                                       ...],
                 stand_positions: Tuple[float]) -> None:
  ...
  '''
    ...
    def _build_urdf_ids(self) -> None:
        ...
        for joint_id in range(num_joints):
            joint_info = self._pybullet_client.getJointInfo(
                self.quadruped, joint_id)
  ...
  '''
    ...
    def reset(self, num_reset_steps: Optional[int] = None) -> None:
        """Resets the robot."""
    ...
    def stand_up(self, num_standup_steps: Optional[int] = None):
        self.change_controller_param([35, 35, 35],[1, 1, 1])
        motor_command = MotorCommand(desired_position=self._stand_positions,
                                     kp=self._motor_group.kps,
                                     desired_velocity=0,
                                     kd=self._motor_group.kds,
  ...
  '''
    ...
    def _apply_action(self, action, motor_control_mode=None) -> None:
        ...
        torques, observed_torques = self._motor_group.convert_to_torque(
            action, self.motor_angles, self.motor_velocities,
            motor_control_mode)
  ...
  '''
    ...
    def change_controller_param(self, kps: Tuple[float], kds: Tuple[float]):
        self._motor_group.kps = np.repeat(kps, self.num_legs)
        print(self._motor_group.kps)
        self._motor_group.kds = np.repeat(kds, self.num_legs)
        print(self._motor_group.kds)
